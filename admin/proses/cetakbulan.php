<?php
    ...
    $pdf->Cell(0,5,'DAFTAR PESANAN BULAN INI','0','1','C',false);
    $pdf->Ln(4);
	...
	$dateb=$thn."-".$bln."-31";
    $no = 0;
    $sql = mysqli_query($con, "SELECT * FROM pesanan,meja WHERE meja_idmeja=idmeja AND tanggal BETWEEN '$datea' AND '$dateb' AND status BETWEEN 'proses' AND 'selesai' ORDER BY tanggal DESC") or die (mysqli_error($con));
    ...
    $pdf->Ln(5);
    $pdf->Cell(0,5,"Jumlah Pesanan bulan ini pada Cafe Papua Code : ".$count." Pesanan",'0','1','C',false);
    $pdf->Line(10, 25, 225-20, 25);
    $pdf->Ln(10);
    // $pdf->AliasNbPages('{totalPages}');
    // $pdf->Cell(0, 5, "Page " . $pdf->PageNo() . "/{totalPages}", 0, 1);
    $pdf->Output('',"Laporan Pesanan Bulan Ini.pdf");
