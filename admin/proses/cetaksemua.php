<?php
    ...
    $pdf->Cell(0,5,'DAFTAR SEMUA PESANAN','0','1','C',false);
    $pdf->Ln(4);
    ...
    $no = 0;
    $sql = mysqli_query($con, "SELECT * FROM pesanan,meja WHERE meja_idmeja=idmeja AND status BETWEEN 'proses' AND 'selesai' ORDER BY tanggal DESC") or die (mysqli_error($con));
    ...
    $pdf->Ln(5);
    $pdf->Cell(0,5,"Jumlah semua Pesanan pada Cafe Papua Code : ".$count." Pesanan",'0','1','C',false);
    $pdf->Line(10, 25, 225-20, 25);
    $pdf->Ln(10);
    // $pdf->AliasNbPages('{totalPages}');
    // $pdf->Cell(0, 5, "Page " . $pdf->PageNo() . "/{totalPages}", 0, 1);
    $pdf->Output('',"Laporan Semua Pesanan.pdf");
